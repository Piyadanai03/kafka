...
redisClient.on('error', (err) => console.error(
  JSON.stringify({ level: "ERROR", service: SERVICE_NAME, message: "Redis Client Error", error: err })
));
// <<< END ADD >>>
...
  const { id: rejectedSchemaId } = await registry.register(orderRejectedSchema);
  ...
  console.log(JSON.stringify({ level: "INFO", service: SERVICE_NAME, message: "Redis connected." }));
...
            traceId,
            orderId,
            message: "Idempotency check (Redis): Order already processed. Skipping.",
          })
        );
